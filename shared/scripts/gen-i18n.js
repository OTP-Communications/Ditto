...
const TRANSLATIONS_FUNCS = ['_t'];

const INPUT_TRANSLATIONS_FILE = 'shared/i18n/locales/en_US.json';
const OUTPUT_FILE = 'shared/i18n/locales/en_US.json';
...
const SEARCH_PATHS = ['mobile', 'web', 'shared'];
...
function getFormatStrings(str) {
  ...
  while ((match = formatStringRe.exec(str)) !== null) {
    ...
    if (placeholderFormat !== 's') {
      throw new Error(
        `'${placeholderFormat}' used as format character: you probably meant 's'`,
      );
    }
            ...
            try {
              ...
              for (const placeholder of placeholders) {
                if (node.arguments.length < 2) {
                  throw new Error(
                    `Placeholder found ('${placeholder}') but no substitutions given`,
                  );
                }
...
function getTranslationsOther(file) {
  ...
  const translationsRegex = /_t\(['"]([\s\S]*?)['"]\)/gm;
  let matches;
...
const inputTranslationsRaw = JSON.parse(
  fs.readFileSync(INPUT_TRANSLATIONS_FILE, {encoding: 'utf8'}),
);
const enPlurals = {};
...
const walkOpts = {
  listeners: {
    ...
    file: function (root, fileStats, next) {
      ...
      let trs;
      if (
        fileStats.name.endsWith('.js') ||
        fileStats.name.endsWith('.ts') ||
        fileStats.name.endsWith('.tsx')
      ) {
